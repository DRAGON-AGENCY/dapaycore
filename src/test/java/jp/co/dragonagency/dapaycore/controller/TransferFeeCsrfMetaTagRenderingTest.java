...
/**
 * 項番41〜43：operation_transfer_fee_edit.html の CSRF meta タグレンダリングを検証するテスト。
 *
 * 確認内容:
 *   operation_transfer_fee_edit.html において Thymeleaf が
 *   th:content="${csrfToken}" を実際のトークン値に置換し、
 *   レンダリング後の HTML の <meta name="csrf-token" content="..."> に
 *   非空のトークン文字列が埋め込まれること。
 *
 * テスト対象: GET /operation_transfer_fee_edit.html
...
class TransferFeeCsrfMetaTagRenderingTest {
    ...
    // =========================================================
    // 項番41：CSRF meta タグに th:content でトークンが埋め込まれる
    // =========================================================

    @Test
    void T41_csrfMetaTag_レンダリングされたHTMLのcontent属性に非空トークンが埋め込まれる()
            throws Exception {
        mockMvc.perform(get("/operation_transfer_fee_edit.html")
    ...
    @Test
    void T42_csrfMetaTag_フォールバックの空文字がそのまま出力されていない() throws Exception {
        mockMvc.perform(get("/operation_transfer_fee_edit.html")
                        .sessionAttr("loginUser", "user001"))
                .andExpect(status().isOk())
                .andExpect(content().string(
    ...
    @Test
    void T43_csrfMetaTag_異なるセッションでは独立したトークンが生成される() throws Exception {
        String html1 = mockMvc.perform(get("/operation_transfer_fee_edit.html")
                        .sessionAttr("loginUser", "user001"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();

        String html2 = mockMvc.perform(get("/operation_transfer_fee_edit.html")
                        .sessionAttr("loginUser", "user001"))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
