...
- **プレースホルダ連動**：選択中の対象に合わせて入力欄の placeholder を
  「◯◯で絞込み」に切り替える。
- **部分一致絞込み**：`normalize()` で正規化して部分一致でフィルタする。空入力なら全件表示に戻す。
  `normalize()` は **`String.prototype.normalize('NFKC')` で全角／半角を同一視**（全角英数字→半角、
  半角カナ→全角カナ 等）してから、**前後・途中の空白（全角スペース含む）を除去し、英字を小文字化**する。
  ```javascript
  ...
  ```
  NFKC を入れないと、法人名（カナ）欄など全角カナの列で「全角で入力した時と半角で入力した時の
  絞込み結果が変わる」不具合になる（2026-09-03 に手数料一覧で発覚し全一覧画面へ NFKC を追加）。
- **絞込みトリガ**：`絞込み`ボタン・入力欄での Enter・対象チェックボックスの切替で実行。
- **クリア**：入力を空にし、対象を初期状態（先頭）へ戻し、**表示件数も初期値（先頭の選択肢＝10 件）へ戻して**
  全件表示・1 ページ目に戻す（`perPageSelect.value = perPageSelect.options[0].value;` と
  `perPage` の再代入を忘れない）。
...
- 列幅クラス（例: `c-code`, `c-name`, `c-amt`）を `<col>` と `<th>` / `<td>` に付与して
  管理する。
- **スクロールラッパー（`.data-scroll` / `.table-wrap` 等）は横方向のみ（`overflow-x: auto`）とし、
  `overflow-y` や `max-height` は設定しない。** 縦は「表示件数」で選択した件数（10/20/30/50/100）を
  そのページで全件表示し、テーブル内スクロールは作らない（ページ全体でスクロールする）。
  `thead th` の `position: sticky; top: 0` はページスクロールに対して機能する。
  リファレンス実装は `operation_transfer_fee_list.html` / `operation_contact_inquiry_list.html`。
